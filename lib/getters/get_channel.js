// TODO : on legacy commands support dropped, delete this file
async function getChannel(msg, args) {
    ...
    if (channel.length === 0) {
        const channelNameOrID = args.join(" ")
        ...
        if (channel === undefined) {
            ...
            if (channel === undefined) {
                const results = channels.filter(channel => channel.name.toLowerCase().indexOf(channelNameOrID.toLowerCase()) >= 0 && ["GUILD_TEXT", "GUILD_NEWS"].includes(channel.type)).slice(0, 10)
