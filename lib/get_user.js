            ...
            if (member === undefined) {
                ...
                if (member === undefined) {
                    ...
                    if (results.length === 1) member = results[0]
                        ...
                        const filter = cMsg => cMsg.author.id === msg.author.id && cMsg.content === "N" || (!isNaN(cMsg.content) && cMsg.content > 0 && cMsg.content <= results.length)
                        try {
                            let cMsg = await msg.channel.awaitMessages(filter, { max: 1, time: 30_000 })
                            cMsg = cMsg.array()
                            if (cMsg.length === 1) {
                                if (cMsg[0].content !== "N") member = results[cMsg[0].content - 1]
