module.exports = {
    ...
    async execute (bot, msg, args) {
        ...
        if (arg === "remove") {
            if (!msg.member.hasPermission("ADMINISTRATOR")) return msg.channel.send(__("missing_permissions_to_remove_channel"))
            const channelArg = args[1]
            if (!channelArg) return msg.channel.send(__("precise_channel_to_remove"))
            ...
            if (!channelRow) return msg.channel.send(__("channel_not_in_db"))
            ...
            msg.channel.send(`${__("the_channel")} ${channel.name} ${__("has_been_removed_to_list")}`)
        }
        else if (arg === "list") {
            ...
            if (channelsRows.length === 10) return msg.channel.send(__("max_channels_count_reached"))

            const addChannelRequest = bot.db.prepare("INSERT INTO xp_channels VALUES(?,?)")
            addChannelRequest.run(msg.guild.id, channel.id)

            msg.channel.send(`${__("the_channel")} ${channel.name} ${__("has_been_added_to_list")}`)
        }
    }
}