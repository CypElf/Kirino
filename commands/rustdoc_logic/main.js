  ...
  function getSearchElement() {
    ...
    function getQueryStringParams() {
      ...
      window.location.search.substring(1).split('&').map(function (s) {
        var pair = s.split('=');
        params[decodeURIComponent(pair[0])] = typeof pair[1] === 'undefined' ? null : decodeURIComponent(pair[1])
      });
      return params
    }
    ...
    function initSearch(rawSearchIndex) {
      ...
      if (savedCrate !== null) {
        onEachLazy(document.getElementById('crate-search').getElementsByTagName('option'), function (e) {
      ...
      function execQuery(query, searchWords, filterCrates) {
        ...
        if ((val.charAt(0) === '"' || val.charAt(0) === '\'') && val.charAt(val.length - 1) === val.charAt(0)) {
          ...
          for (i = 0; i < nSearchWords; ++i) {
            ...
            } else if ((in_args === true || returned === true) && typePassesFilter(typeFilter, searchIndex[i].ty)) {
        ...
        if (ALIASES && ALIASES[window.currentCrate] && ALIASES[window.currentCrate][query.raw]) {
      ...
      function validateResult(name, path, keys, parent) {
        for (var i = 0; i < keys.length; ++i) {
          if (!(name.indexOf(keys[i]) > - 1 || path.indexOf(keys[i]) > - 1 || (parent !== undefined && parent.name.toLowerCase().indexOf(keys[i]) > - 1) || levenshtein(name, keys[i]) <= MAX_LEV_DISTANCE)) {
            return false
          }
        }
        return true
      ...
      function initSearchNav() {
        ...
        search_input.onkeydown = function (e) {
          ...
          if (e.which === 38) {
            if (!actives[currentTab].length || !actives[currentTab][0].previousElementSibling) {
              return
            }
            ...
            if (actives[currentTab].length) {
              document.location.href = actives[currentTab][0].getElementsByTagName('a') [0].href
            }
          } else if (e.which === 9) {
            if (e.shiftKey) {
      ...
      function addTab(array, query, display) {
        ...
        if (array.length > 0) {
          ...
          array.forEach(function (item) {
            ...
            length += 1;
            output += '<tr class="' + type + ' result"><td>' + '<a href="' + item.href + '">' + (item.is_alias === true ? ('<span class="alias"><b>' + item.alias + ' </b></span><span ' + 'class="grey"><i>&nbsp;- see&nbsp;</i></span>')  : '') + item.displayPath + '<span class="' + type + '">' + name + '</span></a></td><td>' + '<a href="' + item.href + '">' + '<span class="desc">' + escape(item.desc) + '&nbsp;</span></a></td></tr>'
          });
          output += '</table>'
        } else {
          output = '<div class="search-failed"' + extraStyle + '>No results :(<br/>' + 'Try on <a href="https://duckduckgo.com/?q=' + encodeURIComponent('rust ' + query.query) + '">DuckDuckGo</a>?<br/><br/>' + 'Or try looking in one of these:<ul><li>The <a ' + 'href="https://doc.rust-lang.org/reference/index.html">Rust Reference</a> ' + ' for technical details about the language.</li><li><a ' + 'href="https://doc.rust-lang.org/rust-by-example/index.html">Rust By ' + 'Example</a> for expository code examples.</a></li><li>The <a ' + 'href="https://doc.rust-lang.org/book/index.html">Rust Book</a> for ' + 'introductions to language features and the language itself.</li><li><a ' + 'href="https://docs.rs">Docs.rs</a> for documentation of crates released on' + ' <a href="https://crates.io/">crates.io</a>.</li></ul></div>'
        }
        return [output,
        length]
      }
      function makeTabHeader(tabNb, text, nbElems) {
        if (currentTab === tabNb) {
          return '<div class="selected">' + text + ' <div class="count">(' + nbElems + ')</div></div>'
        }
        return '<div>' + text + ' <div class="count">(' + nbElems + ')</div></div>'
      }
      function showResults(results) {
        if (results.others.length === 1 && getCurrentValue('rustdoc-go-to-only-result') === 'true') {
        ...
        var output = '<h1>Results for ' + escape(query.query) + (query.type ? ' (type: ' + escape(query.type) + ')' : '') + '</h1>' + '<div id="titles">' + makeTabHeader(0, 'In Names', ret_others[1]) + makeTabHeader(1, 'In Parameters', ret_in_args[1]) + makeTabHeader(2, 'In Return Types', ret_returned[1]) + '</div><div id="results">' + ret_others[0] + ret_in_args[0] + ret_returned[0] + '</div>';
    ...
    window.register_implementors = function (imp) {
      ...
      for (var i = 0; i < llength; ++i) {
        ...
        struct_loop: for (var j = 0; j < slength; ++j) {
          ...
          display.innerHTML = '<span class="in-band"><table class="table-display">' + '<tbody><tr><td><code>' + code.outerHTML + '</code></td><td></td></tr>' + '</tbody></table></span>';
    ...
    function toggleAllDocs(pageId, fromAutoCollapse) {
      ...
      if (hasClass(innerToggle, 'will-expand')) {
        ...
        onEveryMatchingChild(innerToggle, 'inner', function (e) {
          ...
          if (!parent || !superParent || superParent.id !== 'main' || hasClass(parent, 'impl') === false) {
    ...
    function collapseDocs(toggle, mode, pageId) {
      ...
      if (hasClass(toggle.parentNode, 'impl') === false) {
        ...
        if ((!relatedDoc && hasClass(docblock, 'docblock') === false) || (pageId && document.getElementById(pageId))) {
          return
        }
        if (mode === 'toggle') {
          if (hasClass(relatedDoc, 'fns-now-collapsed') || hasClass(docblock, 'hidden-by-impl-hider')) {
            action = 'show'
          } else {
            action = 'hide'
          }
    ...
    function createSimpleToggle(sectionIsCollapsed) {
      ...
      toggle.innerHTML = '[<span class="inner">' + labelForToggleButton(sectionIsCollapsed) + '</span>]';
      return toggle
    }
    ...
    var func = function (e) {
      ...
      if (hasClass(next, 'docblock') === true || (hasClass(next, 'stability') === true && hasClass(next.nextElementSibling, 'docblock') === true)) {
    ...
    newToggle.innerHTML = '[<span class="inner">' + labelForToggleButton(true) + '</span>] Show hidden undocumented items';
    function toggleClicked() {
      if (hasClass(this, 'collapsed')) {
        ...
        this.innerHTML = '[<span class="inner">' + labelForToggleButton(false) + '</span>] Hide undocumented items'
      } else {
        ...
        this.innerHTML = '[<span class="inner">' + labelForToggleButton(true) + '</span>] Show hidden undocumented items'
      }
    }
    onEachLazy(document.getElementsByClassName('impl-items'), function (e) {
      ...
      for (var i = 0; i < hlength; ++i) {
        if (hasClass(hiddenElems[i], 'content') === false && hasClass(hiddenElems[i], 'docblock') === false) {
          needToggle = true;
          break
        }
      }
    ...
    function buildToggleWrapper(e) {
      ...
      if (e.parentNode.id === 'main') {
        ...
        e.parentNode.insertBefore(createToggle(otherMessage, fontSize, extraClass, hasClass(e, 'type-decl') === false || showItemDeclarations === true), e);
    ...
    function showModal(content) {
      ...
      modal.innerHTML = '<div class="modal-content"><div class="close" id="modal-close">✕' + '</div><div class="whiter"></div><span class="docblock">' + content + '</span></div>';
    ...
    function putBackSearch(search_input) {
      if (search_input.value !== '') {
        ...
        if (browserSupportsHistoryApi()) {
          history.replaceState(search_input.value, '', '?search=' + encodeURIComponent(search_input.value))
        }
      }
    }
    if (search_input) {
