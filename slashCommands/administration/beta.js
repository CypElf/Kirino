...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("beta")
        .setDescription(__("description_beta"))
    ...
    async execute(bot, interaction) {
        ...
        if (subcommand === "status") {
            if (isBetaEnabled) {
                interaction.reply(`${__("beta_is_enabled")} ${__("kirino_glad")}`)
            }
            else {
                interaction.reply(`${__("beta_is_disabled")} ${__("kirino_glad")}`)
            }
        }

        else {
            if (interaction.guild && !interaction.member.permissions.has(Permissions.FLAGS.MANAGE_SERVER)) {
                return interaction.reply({ content: `${__("missing_permissions_to_enable_beta")} ${__("kirino_pout")}`, ephemeral: true })
            }

            if (subcommand === "enable") {
                if (isBetaEnabled) {
                    return interaction.reply({ content: `${__("beta_already_enabled")} ${__("kirino_glad")}`, ephemeral: true })
                }

                await interaction.reply(`${__("beta_confirmation")} ${__("kirino_what")}`)
                ...
                collector.on("collect", async reaction => {
                    ...
                    if (reaction.emoji.name === "✅") {
                        ...
                        interaction.editReply(`${__("beta_enabled")} ${__("kirino_glad")}`)
                    }
                    else {
                        confirmationMsg.reactions.removeAll()
                        interaction.editReply(`${__("beta_enabled_cancelled")} ${__("kirino_pout")}`)
                    }
                })
            }
            else if (subcommand === "disable") {
                if (!isBetaEnabled) {
                    return interaction.reply({ content: `${__("beta_already_disabled")} ${__("kirino_glad")}`, ephemeral: true })
                }

                bot.db.prepare("DELETE FROM beta WHERE id = ?").run(id)
                interaction.reply(`${__("beta_disabled")} ${__("kirino_glad")}`)
            }
        }
    }
}