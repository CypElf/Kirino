...
module.exports = {
    ...
    async execute(bot, interaction) {
        ...
        if (subcommand === "add") {
            ...
            if (!interaction.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) return interaction.reply({ content: `${__("not_enough_permissions_to_add_rule")} ${__("kirino_pff")}`, ephemeral: true })

            if (rule.length > 1000) return interaction.reply({ content: `${__("rule_too_long")} ${__("kirino_pout")}`, ephemeral: true })

            const rulesCount = bot.db.prepare("SELECT * FROM rules WHERE guild_id = ?").all(interaction.guild.id).length
            if (rulesCount >= 30) return interaction.reply({ content: `${__("max_rules_number_reached")} ${__("kirino_pout")}`, ephemeral: true })

            bot.db.prepare("INSERT INTO rules(guild_id, rule) VALUES(?,?)").run(interaction.guild.id, rule)

            interaction.reply(`${__("the_following_rule")}\n\`\`\`${rule}\`\`\`\n${__("has_been_added_to_rules")}`)
        }
        ...
        // ------------------------------------------------------------------- remove
            ...
            if (rules.length === 0) return interaction.reply({ content: __("no_rules_defined_on_this_server"), ephemeral: true })

            if (index < 0 || index >= rules.length) return interaction.reply({ content: `${__("no_rules_defined_at_this_index")} ${__("kirino_pout")}`, ephemeral: true })
            ...
            interaction.reply(`${__("the_rule_number_n")} ${index + 1} ${__("has_been_deleted_from_rules")}`)
        }
        ...
        // ------------------------------------------------------------------- count
            ...
            const rulesCount = bot.db.prepare("SELECT * FROM rules WHERE guild_id = ?").all(interaction.guild.id).length
            interaction.reply(`${__("there_is_currently", rulesCount)} ${rulesCount} ${__("rules_on_this_server", rulesCount)}`)
        }
        ...
        // ------------------------------------------------------------------- display rule
            ...
            const ruleEmbed = new MessageEmbed()
                .addField(__("rule_title") + (index + 1), rule)
                .setColor("#000000")
                .setFooter(__("rules_from") + interaction.guild.name, interaction.guild.iconURL())
