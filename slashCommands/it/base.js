...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("base")
        .setDescription(__("description_base"))
    ...
    async execute(bot, interaction) {
        ...
        if (inputBase < 2 || inputBase > 36 || outputBase < 2 || outputBase > 36) {
            return interaction.reply({ content: __("base_out_of_range"), ephemeral: true })
        }

        const convertedToDecimal = parseInt(number, inputBase)
        if (isNaN(convertedToDecimal)) return interaction.reply({ content: __("bad_number"), ephemeral: true })
