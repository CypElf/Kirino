...
export const command: KirinoCommand = {
    builder: new SlashCommandBuilder()
        ...
        .addSubcommandGroup(option => option.setName("channel").setDescription("Manage the channel in which the call result will be sent").addSubcommand(option => option.setName("get").setDescription("Display the currently set channel in which the calls results will be sent")).addSubcommand(option => option.setName("set").setDescription("Change the channel in which the calls results will be sent (need the manage guild permission)").addChannelOption(option => option.setName("channel").setDescription("The new channel in which to send the calls results").setRequired(true))).addSubcommand(option => option.setName("dm").setDescription("Send the call results in the user's DM (need the manage guild permission)")).addSubcommand(option => option.setName("reset").setDescription("Results will be sent in the same channel as the call itself (need the manage guild permission)")))
        .addSubcommand(option => option.setName("asfile").setDescription("Wether the results are sent as a file or as plain text (need the manage guild permission)").addBooleanOption(option => option.setName("as_file").setDescription("Whether to send the file as plain text or as a text file in an attachment").setRequired(true)))
